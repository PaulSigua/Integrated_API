...
def delete_table_facturas():
    try:
        with cursor:
            cursor.execute("""
                    IF EXISTS (SELECT * FROM sysobjects WHERE name='rptUnosof_API_Venture_Facturas_StarFlowers_Dev' AND xtype='U')
                    BEGIN
                        DROP TABLE rptUnosof_API_Venture_Facturas_StarFlowers_Dev
                    END
            """)
            cursor.commit()
...
def create_table_facturas():
    try:
        with cursor:
            cursor.execute("""
                    CREATE TABLE rptUnosof_API_Venture_Facturas_StarFlowers_Dev(
...
def delete_table_cajas():
    try:
        with cursor:
            cursor.execute("""
                IF EXISTS (SELECT * FROM sysobjects WHERE name='rptUnosof_API_Venture_Cajas_StarFlowers_Dev' AND xtype='U')
                    BEGIN
                        DROP TABLE rptUnosof_API_Venture_Cajas_StarFlowers_Dev
                    END
            """)
            cursor.commit()
...
def create_table_cajas():
    try:
        with cursor:
            cursor.execute("""
                    CREATE TABLE rptUnosof_API_Venture_Cajas_StarFlowers_Dev(
                        caj_fact_id INT,
                        caj_id INT PRIMARY KEY,
                        caj_tipo NVARCHAR(100),
                        caj_bodega NVARCHAR(100),
                        caj_cantidad INT,
                        CONSTRAINT FK_caj_fact FOREIGN KEY (caj_fact_id) REFERENCES rptUnosof_API_Venture_Facturas_StarFlowers_Dev(fact_id)
                    )
            """)
            cursor.commit()
...
def delete_table_productos():
    try:
        with cursor:
            cursor.execute("""
                IF EXISTS (SELECT * FROM sysobjects WHERE name='rptUnosof_API_Venture_Productos_StarFlowers_Dev' AND xtype='U')
                    BEGIN
                        DROP TABLE rptUnosof_API_Venture_Productos_StarFlowers_Dev
                    END
            """)
            cursor.commit()
...
def create_table_productos():
    try:
        with cursor:
            cursor.execute("""
                    CREATE TABLE rptUnosof_API_Venture_Productos_StarFlowers_Dev(
                        pro_caj_id INT,
                        pro_id NVARCHAR(100),
                        pro_cantidad INT,
                        pro_cantidad_t INT,
                        pro_precio_t FLOAT,
                        CONSTRAINT FK_pro_caj FOREIGN KEY (pro_caj_id) REFERENCES rptUnosof_API_Venture_Cajas_StarFlowers_Dev(caj_id)
                    )  
            """)
            cursor.commit()
...
url_api_getVen1JSON_query = """SELECT prm_valor
                FROM dbo.Parametros_Sistema
                WHERE id_grupo = 13 AND prm_descripcion = 'url_api_getV1'"""

api_key_getCVen1JSON_query = """SELECT prm_valor
                FROM dbo.Parametros_Sistema
                WHERE id_grupo = 13 AND prm_descripcion = 'api_key_getV1'"""

insert_facturas_query = """INSERT INTO rptUnosof_API_Venture_Facturas_StarFlowers_Dev VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"""

insert_cajas_query = """INSERT INTO rptUnosof_API_Venture_Cajas_StarFlowers_Dev VALUES (?,?,?,?,?)"""

insert_productos_query = """INSERT INTO rptUnosof_API_Venture_Productos_StarFlowers_Dev VALUES (?,?,?,?,?)"""

def get_url_getVen1JSON():
    return get_value_from_db(url_api_getVen1JSON_query)

def get_api_key_getCVen1JSON():
    return get_value_from_db(api_key_getCVen1JSON_query)
